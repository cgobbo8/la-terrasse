...
- [ ] Task 1: Add ARIA attributes to MegaMenu.svelte pole triggers (AC: #6)
  - [ ] Add `aria-haspopup="true"` to each pole trigger `<a>` element
  - [ ] Bind `aria-expanded={activeMenu === pole.id}` on each pole trigger
  - [ ] Add `role="menu"` to each dropdown panel container
  - [ ] Add `role="menuitem"` to each sub-link `<a>` inside the dropdown
  - [ ] Add `aria-haspopup="true"` and `aria-expanded` to the transversal "Experiences" trigger button

- [ ] Task 2: Implement keyboard navigation in MegaMenu.svelte (AC: #4)
  - [ ] Add `onkeydown` handler to pole trigger elements: Enter/Space opens dropdown, Escape closes it
  - [ ] When dropdown opens via keyboard, move focus to the first `role="menuitem"` link
  - [ ] Arrow Down/Up cycles focus between `role="menuitem"` links within the open dropdown
  - [ ] Escape closes the dropdown and returns focus to the trigger that opened it
  - [ ] Tab from the last item in a dropdown moves focus to the next top-level trigger (natural tab order)
  - [ ] Apply the same keyboard behavior to the transversal "Experiences" dropdown

- [ ] Task 3: Add visible focus indicators (AC: #5)
  - [ ] Add `focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:rounded` classes to all top-level trigger elements
  - [ ] Set `focus-visible:outline-color` via inline style using the pole's accent color for pole triggers
  - [ ] Set `focus-visible:outline-color` to `#2D2B1B` (brun-terre) for the transversal trigger and contact button
  - [ ] Add focus-visible ring to all sub-link items inside dropdowns

- [ ] Task 4: Verify current section indicator (AC: #7)
  - [ ] Confirm that `currentPole` prop correctly highlights the active pole label in accent color
  - [ ] Ensure non-active poles display in `#6b6b67` (gray-600) and active pole in its accent color
  - [ ] Verify the color is applied via inline `style` attribute (already implemented, validate correctness)

- [ ] Task 5: Verify complete navigation content (AC: #1, #2, #8)
  - [ ] Confirm all restaurant sub-links render: La carte, Producteurs, Repas de groupe
  - [ ] Confirm all aventure sub-links render: Pedalo, Paddle, Mini-golf, Toutes les activites
  - [ ] Confirm all evenements sub-links render: La salle, Seminaire Simple, Seminaire Gourmet, Seminaire Aventure
  - [ ] Confirm transversal items render: En Famille, En Groupe, En Entreprise with descriptions
  - [ ] Confirm each pole has a featured card with title, description, CTA text
  - [ ] Confirm cross-sell links at bottom of each pole panel point to a different pole
  - [ ] Verify all links use `getLocalizedPath()` in Header.astro when building the data object

- [ ] Task 6: Verify animation and hover delay (AC: #9)
  - [ ] Confirm `megaFadeIn` keyframes exist in `src/styles/global.css` (already present)
  - [ ] Confirm dropdown uses `animate-[megaFadeIn_0.15s_ease-out]` class
  - [ ] Confirm 200ms `setTimeout` delay on `mouseleave` before closing (already implemented via `startClose()`)
