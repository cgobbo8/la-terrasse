...
- [ ] Task 1: Create `src/components/common/ContextualCrossLink.astro` (AC: #5, #6)
  - [ ] Accept props: `text` (string — the narrative sentence), `href` (string — destination link), `pole` (target pole for accent color styling)
  - [ ] Render as a `<div>` with `<a>` inside: the narrative text with an inline link or the entire block as a link
  - [ ] Ghost button style: text rendered in the target pole's accent color, arrow (→) appended, underline on hover
  - [ ] Accent colors applied via inline `style` attribute based on pole prop: restaurant=#2D2B1B, aventure=#537b47, evenements=#3d4969
  - [ ] Typography: italic or regular text, slightly larger than body (text-lg), centered or left-aligned depending on page context
  - [ ] Padding: generous vertical spacing (py-8 to py-12) to let the suggestion breathe
  - [ ] Semantic: use `<aside>` or `<div role="complementary">` since this is supplementary navigation

- [ ] Task 2: Add contextual cross-links to Aventure pole pages (AC: #1, #4, #7)
  - [ ] Aventure hub (`src/pages/aventure/index.astro`): Add ContextualCrossLink after activity grid, before CrossLinkBlock
    - [ ] Text: "Après l'effort, le réconfort — déjeunez au bord du lac avec des produits locaux →"
    - [ ] Href: `/restaurant` (via getLocalizedPath)
    - [ ] Pole: `restaurant` (target pole accent)
  - [ ] Activity detail pages (if they exist): Add page-specific narrative cross-links
    - [ ] Paddle page: "Après votre session paddle, prolongez au restaurant avec vue sur le lac →"
    - [ ] Archery tag page: "Match terminé ? Célébrez autour d'un bon repas entre équipes →"
    - [ ] Each activity page gets a UNIQUE narrative — no copy-paste

- [ ] Task 3: Add contextual cross-links to Restaurant pole pages (AC: #2, #4, #7)
  - [ ] Restaurant hub (`src/pages/restaurant/index.astro`): Add ContextualCrossLink after menu/ambiance section, before CrossLinkBlock
    - [ ] Text: "Prolongez le plaisir — balade ou pédalo sur le lac après le déjeuner →"
    - [ ] Href: `/aventure` (via getLocalizedPath)
    - [ ] Pole: `aventure` (target pole accent)
  - [ ] Menu page (if exists): "Envie de digérer en douceur ? Le lac vous attend pour une balade →"
  - [ ] Group dining page (if exists): "Combinez repas de groupe et activités — la journée parfaite →"

- [ ] Task 4: Add contextual cross-links to Événements pole pages (AC: #3, #4, #7)
  - [ ] Événements hub (`src/pages/evenements/index.astro`): Add TWO ContextualCrossLinks after packages section, before CrossLinkBlock
    - [ ] Cross-link 1 — to Aventure: "Renforcez la cohésion de votre équipe — archery tag, paddle ou course d'orientation en pleine nature →"
    - [ ] Cross-link 1 href: `/aventure`, pole: `aventure`
    - [ ] Cross-link 2 — to Restaurant: "Offrez un déjeuner fait maison à vos équipes — cuisine locale et terrasse au bord du lac →"
    - [ ] Cross-link 2 href: `/restaurant`, pole: `restaurant`
  - [ ] Position both after the packages section, before the CrossLinkBlock

- [ ] Task 5: Write i18n translation keys for cross-link texts (AC: #4)
  - [ ] Create translation keys for each narrative text in FR translation file
  - [ ] Add placeholder EN/ES translations (can be refined later)
  - [ ] Key naming convention: `crosslink.{sourcePage}.{targetPole}` (e.g., `crosslink.aventure.restaurant`, `crosslink.restaurant.aventure`)
